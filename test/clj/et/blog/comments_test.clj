...
(deftest comment-has-own-page
  (let [app (t/make-app)
    ...
    (testing "wrong version returns 404"
      (let [resp (t/GET app "/articles/1/version/2/comment/1")]
        (is (= 404 (:status resp)))))
    (testing "nonexistent comment returns 404"
      (let [resp (t/GET app "/articles/1/version/1/comment/999")]
        (is (= 404 (:status resp)))))))
...
(deftest article-comments-list
  (let [app (t/make-app)
    ...
    (testing "version-specific comments page shows only that version"
      (let [resp (t/GET app "/articles/1/version/1/comments")]
        (is (= 200 (:status resp)))
        (is (str/includes? (:body resp) "Alice"))
        (is (not (str/includes? (:body resp) "Bob"))))
      (let [resp (t/GET app "/articles/1/version/2/comments")]
        (is (= 200 (:status resp)))
        (is (str/includes? (:body resp) "Bob"))
...
(deftest comment-on-older-version
  (let [app (t/make-app)
    ...
    (testing "can comment on older version"
      (let [resp (t/POST app "/articles/1/version/1/comment"
                   {"email" "a@b.com" "display-name" "Old" "body" "On v1"})]
        (is (= 302 (:status resp)))
        (is (= "/articles/1/version/1" (t/redirect-location resp)))))
    (testing "comment form is accessible for older version"
      (let [resp (t/GET app "/articles/1/version/1/comment")]
        (is (= 200 (:status resp)))))))
